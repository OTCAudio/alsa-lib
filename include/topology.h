...
struct snd_tplg_be_dai_template {
 ...
 /** \struct snd_soc_tplg_link_cmpnt_template
 * \brief Template type for Back End link components.
 */
struct snd_tplg_link_cmpnt_template {
	const char *name; /* component name, optional */
	const char *dai_name;
};

 /** \struct snd_tplg_hw_config_template
 * \brief Template type to describe a single BE or CC link runtime supported
...
struct snd_tplg_link_template {
	...
	int id;	/*!< unique ID - used to match with BE and CC links */
	...
	struct snd_tplg_link_cmpnt_template *cpu;       /*!< cpu component */
	struct snd_tplg_link_cmpnt_template *codecs;   /*!< codec components */
	int num_codecs;                 /*!< number of codec components */
